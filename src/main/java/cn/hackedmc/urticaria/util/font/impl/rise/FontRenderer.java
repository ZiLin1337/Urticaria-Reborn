...
import static cn.hackedmc.urticaria.util.font.impl.rise.ColorUtils.reAlpha;
import static org.lwjgl.opengl.GL11.*;

public class FontRenderer extends cn.hackedmc.urticaria.util.font.Font {
...
*/
    ...
    public final int drawStringWithShadow(String newstr, float i, float i1, int rgb) {
        drawString(newstr, i + 0.5, i1 + 0.5, Color.BLACK.getRGB(), true);
    ...
    public final void drawLimitedStringWithAlpha(String text, float x, float y, int color, float alpha, float maxWidth) {
        ...
        for (char c : characters) {
            ...
            if (c != '\247' && (index == 0 || index == characters.length - 1 || characters[index - 1] != '\247')) {
                ...
                glPushMatrix();
                drawString(Character.toString(c), x, y, reAlpha(currentColor, (int) alpha), false);
                glPopMatrix();

                curWidth += (width(Character.toString(c)) * 2.0F);
                x += (width(Character.toString(c)) * 2.0F);
