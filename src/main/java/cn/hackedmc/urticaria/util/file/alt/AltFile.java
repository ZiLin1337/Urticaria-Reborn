package cn.hackedmc.urticaria.util.file.alt;
 ...
 * @since 10/19/2021
 */
public class AltFile extends cn.hackedmc.urticaria.util.file.File {
    ...
    public boolean read() {
        ...
        try {
            ...
            for (Map.Entry<String, JsonElement> jsonElement : jsonObject.entrySet()) {
                ...
                String type = accountJSONElement.has("type") ? accountJSONElement.get("type").getAsString() : "Microsoft";
                Account account = null;
                switch (type) {
                    case "Offline": {
                        String name = accountJSONElement.get("name").getAsString();
                        account = new Account(name, "Offline");
                    ...
                    case "Microsoft": {
                        ...
                        account = new Account(email, password, jsonElement.getKey(), uuid, refreshToken);

                        break;
                    }

                    case "Netease": {
                        String auth = accountJSONElement.get("auth").getAsString();
    ...
    public boolean write() {
        try {
            ...
            for (Account account : Client.INSTANCE.getAltManager().getAccounts()) {
                ...
                moduleJsonObject.addProperty("type", type);
                switch (type) {
                    case "Microsoft": {
                    ...
                    case "Offline": {
                        moduleJsonObject.addProperty("name", account.getEmail());

                        break;
                    }
                    case "Netease": {
                        moduleJsonObject.addProperty("auth", account.getEmail());
