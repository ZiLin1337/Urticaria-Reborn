package cn.hackedmc.urticaria.module.impl.render.footprint;

import cn.hackedmc.urticaria.module.impl.render.Footprint;
...
public class Body extends Mode<Footprint> {
    ...
    private final Listener<Render3DEvent> onRender3D = event -> {
        ...
        for (final PlayerRenderData data : renderData) {
            ...
            final Color color = ColorUtil.withAlpha(ColorUtil.mixColors(getTheme().getSecondColor(), getTheme().getFirstColor(), data.getTick() / getParent().amount.getValue().doubleValue()), (int) (180 * (1 - (data.getTick() / getParent().amount.getValue().doubleValue()))));
