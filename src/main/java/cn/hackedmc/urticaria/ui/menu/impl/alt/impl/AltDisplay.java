package cn.hackedmc.urticaria.ui.menu.impl.alt.impl;
...
public class AltDisplay extends MenuComponent {
    ...
    private static final ResourceLocation CLOSE_RESOURCE = new ResourceLocation("urticaria/icons/close.png");
    ...
    public boolean draw(final double offset, final int mouseX, final int mouseY) {
        ...
        InstanceAccess.NORMAL_RENDER_RUNNABLES.add(() -> {
            ...
            switch (account.getType()) {
                case "Microsoft": {
                    if (this.account.getPassword() == null) return;
                    ...
                    fontRenderer.drawStringWithShadow(this.account.getPassword().replaceAll("\\.", "*"), this.getX() + 40, this.getY() + offset + this.getHeight() / 2.0F + 2, displayColor.getRGB());
                ...
                case "Offline": {
                    fontRenderer.drawStringWithShadow(this.account.getEmail(), this.getX() + 40, this.getY() + offset + this.getHeight() / 2.0F - 10, displayColor.getRGB());
                    fontRenderer.drawStringWithShadow("Offline", this.getX() + 40, this.getY() + offset + this.getHeight() / 2.0F + 2, Color.GRAY.getRGB());

                    break;
                }

                case "Netease": {
                    final String name = this.account.getEmail().length() <= 15 ? this.account.getEmail() : this.account.getEmail().substring(0, 15) + "...";
                    fontRenderer.drawStringWithShadow(name, this.getX() + 40, this.getY() + offset + this.getHeight() / 2.0F - 10, displayColor.getRGB());
                    fontRenderer.drawStringWithShadow("Netease", this.getX() + 40, this.getY() + offset + this.getHeight() / 2.0F + 2, Color.RED.getRGB());
