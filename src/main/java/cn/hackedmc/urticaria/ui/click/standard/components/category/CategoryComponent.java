...
public final class CategoryComponent implements InstanceAccess {
    ...
    public void render(final double offset, final double sidebarWidth, final double opacity, final Screen selectedScreen) {
        ...
        int iconSize = baseSize - 1; // 图标大小
        int textSize = Math.max(10, baseSize - 4); // 文本大小
        // ------------------
        ...
        // 使用动态 textSize 计算宽度
        final double width = FontManager.getNunito(textSize).width(Localization.get(category.getName())) + spacer + category.getFontRenderer().width(category.getIcon());
...
//        }
        ...
        // 使用动态 FontManager.getNunito(textSize) 替代 clickGUI.nunitoSmall
        FontManager.getNunito(textSize).drawString(Localization.get(category.getName()), (float) (x + animation.getValue() / 80f + 3 + spacer) +
                FontManager.getIconsOne(iconSize).width(category.getIcon()), y, color);
    ...
    public void bloom(final double opacity) {
        ...
        int textSize = Math.max(10, baseSize - 4);

        final double width = FontManager.getNunito(textSize).width(Localization.get(category.getName())) + 12;
        RenderUtil.roundedRectangle(x + 1.5, y - 5.5, width + 9, 15, 4,
                ColorUtil.withAlpha(getTheme().getAccentColor(new Vector2d(0, y / 5D)), (int) (Math.min(animation.getValue(), opacity))).darker());
    }
