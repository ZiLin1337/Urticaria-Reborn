...
import cn.hackedmc.urticaria.util.render.GLUtil;
...
public class Gui {
    ...
    public void drawCenteredString(final FontRenderer fontRendererIn, final String text, final int x, final int y, final int color) {
        fontRendererIn.drawStringWithShadow(text, (float) (x - fontRendererIn.width(text) / 2), (float) y, color);
    }
    ...
    public void drawTexturedModalRect(final float x, final int y, final int textureX, final int textureY, final int width, final int height) {
        ...
        worldrenderer.pos(x + 0, y + height, zLevel).tex((float) (textureX) * f, (float) (textureY + height) * f1).endVertex();
        ...
        worldrenderer.pos(x + 0, y, zLevel).tex((float) (textureX) * f, (float) (textureY) * f1).endVertex();
        tessellator.draw();
    }
