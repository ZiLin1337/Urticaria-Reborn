...
public class TextureMap extends AbstractTexture implements ITickableTextureObject, InstanceAccess {
    ...
    public void loadTextureAtlas(final IResourceManager resourceManager) {
        ...
        while (true) {
            if (iterator.hasNext()) {
                final Entry<String, TextureAtlasSprite> entry = (Entry) iterator.next();

                if (!this.skipFirst) {
                    final TextureAtlasSprite textureatlassprite3 = entry.getValue();
                    final ResourceLocation resourcelocation1 = new ResourceLocation(textureatlassprite3.getIconName());
                    ...
                    try {
                        ...
                        if (k3 < j || this.mipmapLevels > 0) {
                            final int i4 = this.mipmapLevels > 0 ? TextureUtils.scaleToGrid(k3, j) : TextureUtils.scaleToMin(k3, j);
                        ...
                        if (texturemetadatasection != null) {
                            ...
                            if (!list1.isEmpty()) {
                                ...
                                final int l1 = abufferedimage[0].getHeight();
                            ...
                            while (iterator1.hasNext()) {
                                final int j4 = ((Integer) iterator1.next()).intValue();

                                if (j4 > 0 && j4 < abufferedimage.length - 1 && abufferedimage[j4] == null) {
                                    final ResourceLocation resourcelocation = this.completeResourceLocation(resourcelocation1, j4);
                    ...
                    i = Math.min(i, Math.min(textureatlassprite3.getIconWidth(), textureatlassprite3.getIconHeight()));
                    final int j3 = Math.min(Integer.lowestOneBit(textureatlassprite3.getIconWidth()), Integer.lowestOneBit(textureatlassprite3.getIconHeight()));

                    if (j3 < k) {
                        logger.warn("Texture {} with size {}x{} limits mip level from {} to {}", new Object[]{resourcelocation2, Integer.valueOf(textureatlassprite3.getIconWidth()), Integer.valueOf(textureatlassprite3.getIconHeight()), Integer.valueOf(MathHelper.calculateLogBaseTwo(k)), Integer.valueOf(MathHelper.calculateLogBaseTwo(j3))});
                        k = j3;
                    }
            ...
            if (k2 < this.mipmapLevels) {
                logger.warn("{}: dropping miplevel from {} to {}, because of minimum power of two: {}", new Object[]{this.basePath, Integer.valueOf(this.mipmapLevels), Integer.valueOf(k2), Integer.valueOf(j2)});
            ...
            final Map<String, TextureAtlasSprite> map = Maps.newHashMap(this.mapRegisteredSprites);
            ...
            for (final TextureAtlasSprite textureatlassprite2 : stitcher.getStichSlots()) {
                ...
                final String s = textureatlassprite2.getIconName();
                map.remove(s);
                ...
                try {
                    ...
                    final CrashReport crashreport1 = CrashReport.makeCrashReport(throwable, "Stitching texture atlas");
                    final CrashReportCategory crashreportcategory1 = crashreport1.makeCategory("Texture being stitched together");
                ...
                for (final TextureAtlasSprite textureatlassprite5 : stitcher.getStichSlots()) {
                    ...
                    if (textureatlassprite6 != null) {
                        if (textureatlassprite6.getIconWidth() <= 0) {
                            ...
                            final List<int[][]> list = textureatlassprite5.getFramesTextureData();
    ...
    public void updateAnimations() {
        ...
        if (Config.isMultiTexture()) {
            for (final TextureAtlasSprite textureatlassprite2 : this.listAnimatedSprites) {
                if (this.isTerrainAnimationActive(textureatlassprite2)) {
                    final TextureAtlasSprite textureatlassprite1 = textureatlassprite2.spriteSingle;
        ...
        if (Config.isShaders()) {
            if (flag) {
                ...
                for (final TextureAtlasSprite textureatlassprite3 : this.listAnimatedSprites) {
    ...
    private int detectMinimumSpriteSize(final Map p_detectMinimumSpriteSize_1_, final IResourceManager p_detectMinimumSpriteSize_2_, final int p_detectMinimumSpriteSize_3_) {
        ...
        for (final Object e : p_detectMinimumSpriteSize_1_.entrySet()) {
            ...
            final ResourceLocation resourcelocation1 = this.completeResourceLocation(resourcelocation);
                        ...
                        if (inputstream != null) {
                            ...
                            if (dimension != null) {
                                ...
                                if (!map.containsKey(Integer.valueOf(j))) {
                                    ...
                                    final int k = ((Integer) map.get(Integer.valueOf(j))).intValue();
        ...
        final Set set1 = new TreeSet(set);
        int l1;

        for (final Iterator iterator = set1.iterator(); iterator.hasNext(); l += l1) {
        ...
        while (iterator1.hasNext()) {
            final int i2 = ((Integer) iterator1.next()).intValue();
            final int j2 = ((Integer) map.get(Integer.valueOf(i2))).intValue();
            k1 += j2;
    ...
    private int[] getMissingImageData(final int p_getMissingImageData_1_) {
        ...
        final BufferedImage bufferedimage1 = TextureUtils.scaleImage(bufferedimage, p_getMissingImageData_1_);
    ...
    private void checkEmissive(final ResourceLocation p_checkEmissive_1_, final TextureAtlasSprite p_checkEmissive_2_) {
        ...
        if (s != null) {
            if (!p_checkEmissive_1_.getResourcePath().endsWith(s)) {
                final ResourceLocation resourcelocation = new ResourceLocation(p_checkEmissive_1_.getResourceDomain(), p_checkEmissive_1_.getResourcePath() + s);
                final ResourceLocation resourcelocation1 = this.completeResourceLocation(resourcelocation);
