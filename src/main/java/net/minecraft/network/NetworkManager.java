...
import net.minecraft.viamcp.network.CompressionReorderEvent;
import net.minecraft.viamcp.network.MCPVLBPipeline;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    public void receivePacketWithDisabler(Packet packet) {
        if (this.channel.isOpen()) {
            try {
                if (Disabler.INSTANCE != null && Disabler.INSTANCE.isEnabled() && Disabler.INSTANCE.grimAC.getValue() && Disabler.INSTANCE.grimACDisabler.post.getValue())
                    if (packet instanceof S32PacketConfirmTransaction)
                        if (GrimACDisabler.inGameTick < 20) GrimACDisabler.inGameTick++;
