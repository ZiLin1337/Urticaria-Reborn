...
import cn.hackedmc.urticaria.module.impl.render.Ambience;
...
public abstract class WorldProvider {
    ...
    /**
     * World chunk bus being used to generate chunks
     */
    ...
    public float[] calcSunriseSunsetColors(final float celestialAngle, final float partialTicks) {
        final float f = 0.4F;
        final float f1 = MathHelper.cos(celestialAngle * (float) Math.PI * 2.0F) - 0.0F;
        final float f2 = -0.0F;

        if (f1 >= f2 - f && f1 <= f2 + f) {
            final float f3 = (f1 - f2) / f * 0.5F + 0.5F;
